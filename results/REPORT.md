# Project Iceberg — Report v11

**Status**: Level 1+2 COMPLETE. All 5 investigations done. Paper-ready.  
**Date**: 2026-04-15  
**Reference**: Odrzywołek (2026), arXiv:2603.21852v2  
**AGENT-DEVIL review**: v3 REVISE → v4-v9 PASS → v10 (INV 1-5 added)
...
= exp(x)                       — ln(L)/ln(L) = 1
```
This holds for any y > 1 with y ≠ e (to avoid ln(ln(y)) = 0, which causes division by zero in the exponent). Confirming Exp(x) is independent of the choice of y. Verified by SymPy (simplify returns 0 difference) and numerically (y=5, x=γ: result matches exp(γ) to 10 digits).
...
### Empirical benchmarks (INV-3)

Measured on x86_64 (Windows 11, Rust 1.94.1, release build, opt-level=3). Each function evaluated 1M-5M times with varying inputs to defeat constant folding.

**Raw operator (single evaluation):**

| Operator | ns/op | Ratio vs EML |
|----------|-------|-------------|
| EML | 30.5 | 1.00x |
| EDL | 29.9 | 0.98x |
| DivLogExp | 30.2 | 0.99x |
| PowerLogInv | 42.2 | **1.38x** |

EML, EDL, DivLogExp are indistinguishable (~30 ns). The synthetic model predicted EML 17% cheaper than EDL/DLE — **the real difference is <2%**, because modern x86 FPUs pipeline exp/ln/div at similar throughput. PowerLogInv is 38% slower due to `powf()`, matching the synthetic prediction of 1.39x.

**Bootstrapping chain (derived functions):**

| Function | EML (ns) | PLI (ns) | PLI/EML |
|----------|----------|----------|---------|
| exp(x) | 9.8 | 108.0 | **11.0x** |
| ln(x) | 73.1 | **24.0** | **0.33x** |
| plus(a,b) | 123.3 | 256.0 | 2.08x |
| times(a,b) | 169.5 | 333.7 | 1.97x |
| div(a,b) | 265.4 | 415.0 | 1.56x |

**Key finding**: PLI derives ln(x) **3x faster** than EML (1 operator call vs 3), but exp(x) **11x slower** (3 nested calls vs 1). For arithmetic chains that rely heavily on exp (the majority), EML dominates. PLI is competitive only for logarithm-heavy workloads.

**Synthetic vs empirical**: The synthetic model correctly predicts the PLI raw operator ratio (1.39x synthetic vs 1.38x measured) but overstates the EML-vs-EDL difference (17% synthetic vs <2% measured). The synthetic model should be treated as a rough ordering guide, not a precise cost predictor.

---

## 6A. The 13/35 Barrier (INV-1)

The highest-scoring non-Sheffer operator is Divide_ArcCos_Cos (arccos(x)/cos(y)) at 14/35. Why can't non-exp/ln operators break through to Sheffer?

### Coverage heatmap

See `results/analysis/coverage_heatmap.png` — 20×35 matrix of top partial hits vs targets.

### 6 absolute bottleneck targets

These targets were not generated by ANY of the top 20 partial hits: **ArcSinh, ArcTan, LogisticSigmoid, Sinh, Tan, binary Log**. These require both exponential and multiplicative structure to derive.

### The 13/35 barrier explained (Subtract_Cos_ArcSin + const=0)

Found (13): constants {-1, 1, 2, EulerGamma, Glaisher, Pi}, unary {ArcCos, ArcSin, Cos, Minus, Sin}, binary {Plus, Subtract}.

Missing (22): split into three blocked categories:
1. **No exponential bridge**: Cannot reach Exp, Log, e, or any hyperbolic function — cos/arcsin are algebraically isolated from the exponential tower
2. **No multiplicative structure**: Cannot produce Times, Divide, Power, Inv, Sqr, Sqrt, Half, Avg, Hypot — subtraction alone cannot generate multiplication
3. **Trig isolation**: Even Tan and ArcTan are blocked because tan = sin/cos requires division

**Theoretical explanation**: The operator cos(x) - arcsin(y) generates a subalgebra closed under addition and trigonometric identities, but not under multiplication or exponentiation. The closure of {cos, arcsin, +, -} over the reals produces a module over Z (integer combinations of trig values), not a ring or field. Without access to exp/ln — which bridge the additive and multiplicative structures via exp(a+b) = exp(a)·exp(b) — no amount of composition can escape this additive closure. This is why the exp-ln pair appears in every known Sheffer operator: it is the unique bridge between addition and multiplication in the elementary function hierarchy.

---

## 6B. Deep Tree Precision Cascade (INV-5)

The true precision cost of Sheffer operators: computing sin(1.0) via the full bootstrapping chain.

### Chain: constant → exp/ln → arithmetic → i → pi → complex exp → sin

| Operator | sin(1.0) via chain | Error | Digits correct |
|----------|-------------------|-------|----------------|
| EML | 0.8414709848078965 | 0.0 | **16.0** |
| EDL | 0.8414709848078965 | 0.0 | **16.0** |
| DivLogExp | 0.8414709848078965 | 0.0 | **16.0** |
| PowerLogInv | 0.8414709848078964 | 1.1e-16 | **15.9** |

**All 4 operators compute sin(1.0) to machine precision (15.9-16.0 digits).** The Euler formula chain exp(ix)-exp(-ix))/(2i) is numerically very stable. PLI loses 0.1 digit due to its 3-nested exp chain accumulating one ULP of error.

### Intermediate checkpoints

| Function | EML digits | PLI digits |
|----------|-----------|-----------|
| exp(1) | 16.0 | 16.0 |
| ln(2) | 15.8 | 16.0 |
| 1+1 | 16.0 | 14.7 |
...
PLI's arithmetic (1+1, 2×3) shows more error (14.7-15.1 digits) because each arithmetic operation passes through the deep exp chain. But the final sin(1.0) recovers to 15.9 digits — the Euler formula is self-correcting.

---

## 6C. PLI Family Hierarchy (INV-2)

### Depth classification of Sheffer operators

| Depth | Derivative structure | Example | Sheffer? |
|-------|---------------------|---------|----------|
| 0 | Rational (no transcendental) | 1/(x-y) | No (Liouville) |
| 1 | Contains exp/ln | EML, EDL | **Yes** |
| 2 | Contains ln(ln(x)) | PLI | **Yes** |
| 3 | Contains ln(ln(ln(x))) | ln(ln(x))^(1/y) | **No** (≤6/35) |

### Depth-3 candidate results (K_max=7)

| Candidate | Const | Found |
|-----------|-------|-------|
| ln(ln(x))^(1/y) | 1 | 3/35 |
| ln(ln(x))^(1/y) | E | 4/35 |
| ln(x)^(ln(y)) | E | 5/35 |
...
No depth-3 candidate exceeds 6/35. The Sheffer property appears limited to depths 1-2.

---

## 6D. Constant-Free Operator Search (INV-4)

### op(x,x) = constant test

| Operator | op(x,x) | Constant? |
|----------|---------|-----------|
| EML | exp(x)-ln(x) | No (x-dependent) |
| PLI | ln(x)^(1/x) | No (x-dependent) |
| Times_Inv_Minus | (1/x)·(-x) = -1 | Yes, but only 9/35 |
| Times_Id_Inv | x·(1/x) = 1 | Yes, but only 9/35 |
| LogBase_Exp_Exp | log_{exp(x)}(exp(x)) = 1 | Yes, but only 9/35 |

Operators that produce constants from op(x,x) exist, but all score ≤9/35 — far from Sheffer. The ★★★ goal (constant-free Sheffer) remains unachieved. This is consistent with the paper's own statement: "Whether a univariate Sheffer exists... remains open."
...
├── cost/empirical_benchmark.json   ← Rust empirical cost data (INV-3)
├── analysis/coverage_heatmap.png   ← 20×35 partial hit heatmap (INV-1)
├── analysis/barrier_analysis.md    ← 13/35 barrier explanation (INV-1)
├── analysis/sin_cascade.json       ← Deep tree sin(1.0) precision (INV-5)
...
2. ~~PLI SymPy chain verification~~ DONE (5/5 PASS + Exp(x) derivation verified by SymPy in Section 4A)
...
*Project Iceberg — Report v11 (text fixes, paper-ready)*
