...
\end{equation}
together with the companion constant~$1$, generates all 36~elementary function primitives (Table~1 of~\cite{odrzywołek2026}) of a standard scientific calculator. The author also identified two additional operators---$\mathrm{edl}(x,y) = e^x / \ln y$ (with constant~$e$) and $-\mathrm{eml}(x,y) = \ln x - e^y$ (with constant~$-\infty$)---and conjectured these were ``the tip of the iceberg.''

Goldstern~\cite{goldstern2010} proved that for any infinite set and any countable collection of operations, a single binary operation generating all of them exists (see also Tao~\cite{tao_mo}). EML is thus one member of an abundant family. However, no second \emph{explicit, elementary-function} Sheffer operator was known, and no systematic census of the search space had been performed.
...
\item \textbf{Level~2}: Three types of nested composition---left nesting $b(u_1(u_2(x)), u_3(y))$, right nesting $b(u_1(x), u_2(u_3(y)))$, and outer wrapping $u_0(b(u_1(x), u_2(y)))$---yielding 120{,}700 candidates. A 3-stage screening pipeline (Python $K_{\max}{=}3$ sieve, requiring derivation of at least one of the four targets $\{e^{\gamma}, \ln\gamma, \gamma{+}A, \gamma{\cdot}A\}$ within $K_{\max}{=}3$; candidates failing this screen cannot plausibly reach 35/35 at higher~$K$ $\to$ Rust $K_{\max}{=}7$ verification $\to$ equivalence analysis) reduces this to 635 candidates for full verification.
\end{itemize}

The 20~unary functions are not chosen by us but match verbatim the unary primitives of Odrzywołek~\cite{odrzywołek2026} (Table~1, the established reference set for this problem): $\exp$, $\ln$, $\sin$, $\cos$, $\tan$, $\arcsin$, $\arccos$, $\arctan$, $\sinh$, $\cosh$, $\tanh$, $\mathrm{arsinh}$, $\mathrm{arcosh}$, $\mathrm{artanh}$, $\sqrt{\cdot}$, $(\cdot)^2$, $1/(\cdot)$, $-(\cdot)$, $(\cdot)/2$, and the logistic sigmoid~$\sigma$. This library is complete in the sense that every standard scientific-calculator unary function is either in this set or derivable from it; extending the library with composed unary functions (e.g., $\exp \circ \ln$) would produce candidates already covered by the Level~2 search.
...
PLI and EML exhibit a mirror-image structure: EML derives $e^x$ at $K{=}3$ and $\ln x$ at $K{=}6$; PLI reverses this ordering.
...
\item[(b)] \emph{Post-compositional equivalence} ($\mathrm{op}_1 = h \circ \mathrm{op}_2$ for elementary~$h$): separated by tower-direction analysis (Theorem~\ref{thm:tower_separation}, Section~5.5), which shows EML and PLI generate partial-evaluation germs of opposite exponential height.
...
\smallskip
\noindent The notion of \emph{topological conjugacy}---existence of a homeomorphism $\phi$ with $\mathrm{op}_1 = \phi^{-1} \circ \mathrm{op}_2 \circ \phi$---sits between levels~(b) and~(c) and constitutes a natural deeper equivalence. Resolving topological conjugacy of PLI and EML requires methods from Picard--Vessiot theory and Hardy field model theory beyond the scope of a discovery-oriented paper. We treat this as an explicit open problem.
...
Among all binary operators of the form $b(u_1(x), u_2(y))$ (Level~1) and their nested compositions (Level~2), with $b \in \{+,-,\times,/,\wedge,\log\}$ and $u_1, u_2$ from the 20~standard unary functions, there are exactly three non-equivalent Sheffer-complete families: EML, EDL, and PLI. No other candidate in the search space achieves more than 14/35 targets.
\end{theorem}
...
\begin{remark}[Computational proof methodology]
Theorem~\ref{thm:threefamily} is established in the epistemic style of computer-verified theorems such as the Four-Color Theorem~\cite{appel_haken_1976}: a finite, exhaustively enumerable space is verified by a deterministic algorithm. We do not claim such verification establishes the general conjecture (Conjecture~\ref{conj:threefamily}); the latter requires structural classification methods beyond exhaustive search.
\end{remark}

\begin{conjecture}[General three-family completeness]\label{conj:threefamily}
We conjecture that EML, EDL, and PLI are the only Sheffer-complete families over the elementary functions at any structural level. This is supported by: (i)~the necessity of $\exp/\ln$ (Theorem~\ref{thm:expln_necessary}) and (ii)~the combinatorial constraint that only three non-commutative binary operations $\{-,/,\wedge\}$ can serve as the outer operation in a Sheffer operator built from $\exp$ and~$\ln$. The Level~1--2 census (Theorem~\ref{thm:threefamily}) provides computational evidence consistent with this conjecture but does not constitute proof of the general statement.
\end{conjecture}
...
\smallskip
A natural objection is whether some non-exp/ln transcendental function might serve as an alternative additive--multiplicative bridge---for instance, hyperbolic identities such as $\sinh(x+y) = \sinh(x)\cosh(y) + \cosh(x)\sinh(y)$ relate addition to multiplication. However, all such elementary candidates ultimately reduce to $\exp/\ln$ by definition: $\sinh(x) = (e^x - e^{-x})/2$, $\cosh(x) = (e^x + e^{-x})/2$, and similarly for trigonometric and inverse functions over~$\mathbb{C}$. Within the elementary function class, $\exp$ and $\ln$ are the only algebraically independent transcendentals (Liouville--Ritt tower; see Theorem~\ref{thm:expln_necessary}). The conjecture therefore restricts to the precise statement: \emph{no elementary operator without $\exp/\ln$ in its bootstrapping chain achieves Sheffer-completeness}.
...
Hardy field exponential height $\mathrm{eh}(f)$ is defined for single-variable germs $f$ at $+\infty$. We extend the analysis to multivariate operators via \emph{partial evaluation}.

For an iterated composition such as $\mathrm{PLI}(\mathrm{PLI}(x,y),z)$, fix $y, z$ to specific values $y_0, z_0 > 1$ with $y_0 \neq e$, yielding the single-variable germ
\[
g_{y_0, z_0}(x) := \mathrm{PLI}(\mathrm{PLI}(x, y_0), z_0).
\]
This germ lies in a Hardy field extension of $\mathbb{R}(x)$ obtained by adjoining the relevant transcendental elements~\cite{vdDMM2001}. The exponential height $\mathrm{eh}(g_{y_0, z_0})$ is well-defined, and direct computation shows that each iteration of PLI strictly decreases this height (introducing one additional logarithmic nesting per iteration).

The analogous construction for EML yields germs $h_{y_0, z_0}(x)$ whose exponential height strictly \emph{increases} with each iteration.

The non-equivalence claim then reads: there exists no elementary post-composition $H$ such that $H \circ \mathrm{EML}$ and PLI yield partial-evaluation germs of identical exponential height. Since elementary post-composition shifts $\mathrm{eh}$ by a finite integer, and EML iteration drives $\mathrm{eh} \to +\infty$ while PLI drives $\mathrm{eh} \to -\infty$, no finite $H$ can bridge the gap.
...
\end{enumerate}
This subsumes the $T_1$--$T_5$ non-equivalence proof (Theorem~\ref{thm:nonequiv}) as a special case.
\end{proof}
...
Mitigating PLI's high failure rate ($55.4\%$, primarily from negative bases with non-integer exponents) would require either restriction to the principal branch of the complex power function or explicit branch-cut handling. We do not pursue these mitigations here, since PLI's contribution lies in structural taxonomy rather than numerical practicality.
...
\item Numerical verification uses double-precision floating point with tolerance $\varepsilon = 16\,\varepsilon_{\mathrm{mach}}$. Near branch cuts, this threshold may produce false positives (spurious target matches) or false negatives (missed valid derivations). The 37/38 mpmath 128-bit cross-validation for PLI mitigates but does not eliminate this risk.
\end{enumerate}
...
We discovered PowerLogInv, $\mathrm{pli}(x,y) = (\ln x)^{1/y}$, the first \emph{explicit, elementary-function} Sheffer operator proven non-equivalent to the EML family under the $T_1$--$T_5$ equivalence criteria. We do not advocate PLI for computational use. Its discovery establishes that the Sheffer property admits structurally distinct families beyond the subtraction/division paradigm, expanding the operator taxonomy without claiming practical superiority. A computational census of 14{,}712 verification runs across two structural levels identifies three operator families within the explored space. The 13/35~barrier analysis suggests that the exp-ln homomorphism is the essential structural ingredient for Sheffer-ness, explaining why all known Sheffer operators are built from exponentials and logarithms.

%======================================================================
\paragraph{Data availability.} Code and data at: \url{https://github.com/iae-control/project-iceberg}.
...
\appendix
\section{Non-Equivalence Verification Details}

The 48~symbolic comparisons of Theorem~\ref{thm:nonequiv} were generated as follows. Six PLI variants were tested:
...
Each was compared against 8~target forms (EML, EDL, $-$EML, DLE, and their argument-swapped versions) via SymPy \texttt{simplify(variant - target)}, yielding $6 \times 8 = 48$ comparisons. All returned nonzero symbolic expressions. Supplementary checks for $T_4$ (constant shift) and $T_5$ (constant multiply) verified that $\mathrm{pli} - \mathrm{target}$ and $\mathrm{pli}/\mathrm{target}$ are non-constant functions at test points $(x,y) = (\gamma, A)$ and $(x,y) = (\sqrt{2}, \sqrt{3})$.

Full SymPy output is available in the project repository.
...
M.~Goldstern, ``A single binary function is enough,'' unpublished note, TU Wien, 2010. \url{https://dmg.tuwien.ac.at/goldstern/www/papers/notes/singlebinary.pdf}
...
\bibitem{appel_haken_1976}
K.~Appel, W.~Haken, ``Every planar map is four colorable,'' \emph{Bull.\ Amer.\ Math.\ Soc.}, vol.~82, pp.~711--712, 1976.
